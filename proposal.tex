...
We have clear picture of our research plan for the near future in order to get basic results with the new schedule frame fork.
In summary, our research plan is as follows.
\begin{itemize}
	\item Choose the appropriate development environment. Done (APM based drones).
	\item Be familiar with the development environment, and make sure that we are able to adapt the system to our needs. Done.
	\item Plan all the control components in order to achieve our case study goals. Done (see Section~\ref{sec:estimator}, Section~\ref{sec:sensors} and Section~\ref{sec:control}).
	\item Plan the new scheduler detail behavior, and it's interaction with the control components. Done (see Section~\ref{sec:scheduler}).
	\item Detail planing of the scheduler interface implementation and adaptation in the code. Done.
	\item Implement the new scheduler and replace the original APM scheduler (manually transform each task periodical requirements to automata based requirements). Expected two weeks.
	\item Develop and implement the vision based sensor component. We plane to base on the buit-in Matlab vision abilities and use SimuCopter~\cite{SimuCopter} to integrate it in the APM program (see below). Expected 1-2 months.
	\item Grub statistics data and fine-tuning the vision component. Expected one month.
	\item Adjust the state estimator as explain in Section~\ref{sec:estimator}, and combine all together using the new scheduler. Expected two weeks.
	\item Check with a wide range of environmental conditions and a wide range of requirements (automatas), and of course grub and summarize the results. Expected two months.
	\item Write the thesis report. Expected two months.
\end{itemize}
