...
%DONE Gera: some bla-bla about the current state of the art and how we will improve it. 
The current state-of-the-art solution of involving heavy computational tasks, e.g vision, in the control system is simply by adding computational power to the system (use faster processor), usually lots more than needed in order to eliminate the chance of loosing predictability.
Some more conservative control engineers prefers to isolate the heavy computation from the core control loop by allocating one of the processor's core for that task, or even run the vision processing on different computer (See how APM suggest to use computer vision abilities, Section~\ref{sec:APM_suported_vision})
...
\begin{figure}[h]
    ...
    \caption{The controller framework we will implement, the \textit{scheduler} will allocate CPU time ($<time,cov>$) for the \textit{Computer Vision} task base on the state estimation certainty using guarded Automata.
...
Each infinite path on the composed automata is a scheduling that satisfy all the tasks requirements. To simplify, We will assume that the resource is allocated in discrete slots of some fixed duration in the style of time-triggered architecture~\cite{RTComposer}. %TODO - Hodai: fix this centence
...
\begin{figure}[]
    ...
    \begin{tikzpicture}[node distance=4cm,auto]
        \node (A) [state, accepting, initial] {};
        \node (B) [state, accepting] [right of=A] {};
        \node (C) [state, accepting] [right of=B] {};
        
        \path[->] (A) edge [loop above] node {$true \wedge s_{slow}$} (A);
        \path[->] (A) edge [bend left] node {$\neg est_{Bad} \wedge s_{fast}$} (B);
        \path[->] (B) edge [bend left] node {$true \wedge s_{slow}$} (A);
        %\path[->] (B) edge [loop above] node {$\neg cov3$} (B);
        \path[->] (B) edge [bend left] node {$est_{good} \wedge s_{fast}$} (C);
        %\path[->] (C) edge [loop above] node {$cov1$} (C);
        \path[->] (C) edge [bend left=60] node {$true \wedge s_{slow}$} (A);
...
\begin{figure}[]
    ...
    \begin{tikzpicture}[node distance=4cm,auto]
        ...
        \path[->] (A) edge [loop above] node {$s_{slow} \wedge est_1$} (A);
        %\path[->] (A) edge [bend left] node {$s_{fast} \wedge est_2$} (B);
        %\path[->] (B) edge [loop above] node {$s_{fast} \wedge est_2$} (B);
        \path[->] (A) edge [loop right] node {$s_{fast} \wedge est_2$} (A);
        %\path[->] (B) edge [bend left] node {$s_{slow} \wedge est_1$} (A);
